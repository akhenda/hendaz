...
export const getFiles = query({
  ...
  async handler(ctx, args) {
    ...
    const _query = args.query;

    if (_query) files = files.filter((file) => file.name.toLowerCase().includes(_query.toLowerCase()));
    ...
    if (args.type) files = files.filter((file) => file.type === args.type);

    const filesWithUrl = await Promise.all(
      files.map(async (file) => ({ ...file, url: await ctx.storage.getUrl(file.fileId) })),
    );
...
function assertCanDeleteFile(user: Doc<'users'>, file: Doc<'files'>) {
  ...
  if (!canDelete) throw new ConvexError('You have no acces to delete this file');
}
